...
{basedir}/runtime/tabs/
    ...
    {tab_id}/
        ...
        screenshot.sh          # executable script — captures tab screenshot
        screenshot.png         # full-resolution tab screenshot (appears after screenshot.sh runs)
...
Each tab directory includes `screenshot.sh`, a self-contained shell script that captures the tab's content at full screen resolution (e.g. 1920x1080) and saves it to `screenshot.png`. Generated by `_write_screenshot_script()` with hardcoded paths.

For full technical reference (C++ capture pipeline, focus suppression, every file and function), see **[screenshot.md](screenshot.md)**.

### How It Works

1. Script removes any existing `screenshot.png`
2. Sends `:tab-screenshot <tab_id>` (or `:tab-screenshot <tab_id> --window`) to qutebrowser via IPC (`--target tab-silent`)
3. Polls for `screenshot.png` (0.5s intervals, configurable timeout)
4. On result: displays file size and resolution

### `:tab-screenshot` Command

Registered in `commands.py` as `tab_screenshot`. Accepts `tab_id` (required) and optional `--window` flag. Searches current window first, then other windows via `objreg.window_registry`. Delegates to `TabRuntimeManager.screenshot_tab()`.

For background tabs, the implementation temporarily switches the QStackedLayout to the target tab (required for Qt's render loop to send BeginFrame), but uses C++ focus suppression and immediate re-show of the original tab to avoid disrupting keyboard input. See [screenshot.md](screenshot.md) for details.

### IPC Commands

| Command | Args | Delegates to |
|---------|------|-------------|
| `:tab-screenshot` | `tab_id`, optional `--window` | `TabRuntimeManager.screenshot_tab()` |

### Quick Usage

```bash
# Screenshot a tab at full screen resolution (1920x1080)
~/.runtime/qutebrowser-yeyito/runtime/tabs/0/screenshot.sh

# Screenshot the whole window instead
~/.runtime/qutebrowser-yeyito/runtime/tabs/0/screenshot.sh --window

# With custom timeout
~/.runtime/qutebrowser-yeyito/runtime/tabs/0/screenshot.sh --timeout 20
```

## Lifecycle

1. **Init**: wipes `tabs/` with `shutil.rmtree` (handles crash leftovers), recreates it
2. **Tab open**: `_on_new_tab` creates `{tab_id}/` dir, populates `tab-data.info`, writes `snapshot-dom.sh`, `console.sh`, `network.sh`, `command.sh`, `screenshot.sh`, wires signals, calls `_update_indices`
...
| File | Role |
|------|------|
| `qutebrowser/browser/tabruntime.py` | `TabRuntimeManager` class — all runtime tab logic, DOM snapshot, network queries, command eval, tab screenshots |
| `qutebrowser/browser/commands.py` | `:dom-snapshot`, `:js-eval-tab`, `:command-eval`, `:tab-screenshot`, `:network-list`, `:network-detail`, `:network-body`, `:network-ws-frames` commands (cross-window tab lookup) |
