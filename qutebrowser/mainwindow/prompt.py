...
from qutebrowser.utils import urlmatch, urlutils, standarddir


prompt_queue = cast('PromptQueue', None)
PERMISSIONS_FILE = 'permissions.py'
PERMISSIONS_HEADER = '# File automatically generated by qutebrowser\n\n'


def _save_permission_to_pyfile(option: str, pattern_str: str, value: bool) -> None:
    """Save a permission decision to the permissions.py file.

    This creates or updates a permissions.py file in the config directory
    that can be sourced by config.py for persistent permission decisions.

    Args:
        option: The config option name (e.g., 'content.notifications.enabled')
        pattern_str: The URL pattern string (e.g., 'https://example.com/*')
        value: The permission value (True or False)
    """
    import pathlib
    import re

    config_dir = pathlib.Path(standarddir.config())
    permissions_file = config_dir / PERMISSIONS_FILE

    # Key for deduplication: (option, pattern)
    entries: dict[tuple[str, str], bool] = {}

    # Read existing entries if file exists
    if permissions_file.exists():
        content = permissions_file.read_text(encoding='utf-8')
        # Parse existing config.set() calls
        # Pattern: config.set('option', True/False, 'pattern')
        pattern = re.compile(
            r"config\.set\(['\"]([^'\"]+)['\"],\s*(True|False),\s*['\"]([^'\"]+)['\"]\)"
        )
        for match in pattern.finditer(content):
            opt, val, pat = match.groups()
            entries[(opt, pat)] = val == 'True'

    # Add or update the new entry
    entries[(option, pattern_str)] = value

    # Write the file
    lines = [PERMISSIONS_HEADER]
    for (opt, pat), val in sorted(entries.items()):
        lines.append(f"config.set('{opt}', {val}, '{pat}')\n")

    permissions_file.write_text(''.join(lines), encoding='utf-8')
...
class YesNoPrompt(_BasePrompt):
    ...
    def accept(self, value=None, save=False):
        ...
        if save:
            ...
            # Also save to permissions.py for users with load_autoconfig(False)
            _save_permission_to_pyfile(opt.name, str(pattern), self.question.answer)
