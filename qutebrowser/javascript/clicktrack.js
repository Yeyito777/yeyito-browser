...
window._qutebrowser.clicktrack = (function() {
    const funcs = {};

    const MARK_ATTR = "data-qutebrowser-clicktrack";
    ...
    function patch_add_event_listener() {
        ...
        if (!proto || proto.__qute_clicktrack_patched) {
            return;
        }
        ...
        Object.defineProperty(proto, "__qute_clicktrack_patched", {
            value: true,
            enumerable: false,
            configurable: false,
            writable: false,
    ...
    function dom0_click_types(elem) {
        const types = new Set();
        for (const eventName of CLICK_EVENTS) {
    ...
    function framework_click_types(elem) {
        const types = new Set();
        let propertyNames;
        try {
    ...
    function collect_types(elem) {
        ...
        for (const type of dom0_click_types(elem)) {
            types.add(type);
        }

        for (const type of framework_click_types(elem)) {
            types.add(type);
        }
    ...
    funcs.mark_frames = () => {
        ...
        function mark_window(win) {
            let count = 0;
            try {
                const helper = win._qutebrowser && win._qutebrowser.clicktrack;
