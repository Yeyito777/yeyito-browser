...
void QWebEnginePage::captureScreenshot(
    const QSize &outputSize,
    const std::function<void(const QByteArray&)> &resultCallback)
{
    Q_D(QWebEnginePage);
    d->ensureInitialized();
    d->captureScreenshot(outputSize, resultCallback);
}

void QWebEnginePagePrivate::captureScreenshot(
    const QSize &outputSize,
    std::function<void(const QByteArray &)> callback)
{
    adapter->captureScreenshot(outputSize, std::move(callback));
}

void QWebEnginePage::suppressFocusNotifications()
{
    QtWebEngineCore::RenderWidgetHostViewQtDelegateClient::s_suppressFocusCount++;
}

void QWebEnginePage::restoreFocusNotifications()
{
    auto &count = QtWebEngineCore::RenderWidgetHostViewQtDelegateClient::s_suppressFocusCount;
    if (count > 0)
        count--;
}
