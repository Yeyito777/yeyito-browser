...
public:
    ...
    // When > 0, focus and keyboard events are suppressed in both the delegate
    // item and client layers.  Used during screenshot capture to prevent
    // transient tab switches from disrupting the active tab's input state.
    static int s_suppressFocusCount;
