...
int RenderWidgetHostViewQtDelegateClient::s_suppressFocusCount = 0;
...
{
    // During screenshot capture, silently accept focus events without
    // notifying Chromium. This prevents tab switches (needed for rendering)
    // from disrupting the renderer's focus/IME state on the active tab.
    if (s_suppressFocusCount > 0) {
        event->accept();
        return;
    }
