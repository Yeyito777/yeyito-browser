...
void WebContentsAdapter::captureScreenshot(
    ...
    std::function<void(const QByteArray &)> callback)
{
    ...
    auto *view = m_webContents->GetRenderWidgetHostView();
    if (!view) {
        callback(QByteArray());
        return;
    }

    auto *rwhvqt = static_cast<RenderWidgetHostViewQt *>(view);

    // 1. Override viewport → renderer lays out at full-screen size
    rwhvqt->m_captureSizeOverride = gfx::Rect(0, 0, outputSize.width(), outputSize.height());
    rwhvqt->synchronizeVisualProperties(std::nullopt);

    // 2. Keep tab rendering (prevents discard, no JS visibility change)
    auto capturer = std::make_shared<base::ScopedClosureRunner>(
        m_webContents->IncrementCapturerCount(
            gfx::Size(), /*stay_hidden=*/true,
            /*stay_awake=*/false, /*is_activity=*/false));

    quint64 reqId = m_nextRequestId++;
    m_screenshotCallbacks.emplace(reqId,
        ScreenshotRequest{std::move(callback), capturer});

    auto weak = sharedFromThis().toWeakRef();

    // 3. Force renderer to produce a frame at the new viewport size
    rwhvqt->host()->ForceRedrawForTesting();

    // 4. Wait for frame, then copy
    rwhvqt->host()->InsertVisualStateCallback(
        base::BindOnce([](QWeakPointer<WebContentsAdapter> w, quint64 id, QSize sz, bool) {
            // Small delay for compositor to process the submitted frame
            QTimer::singleShot(80, [w, id, sz]() {
                auto a = w.toStrongRef();
                if (!a)
                    return;
                if (a->m_screenshotCallbacks.find(id) == a->m_screenshotCallbacks.end())
                    return;

                auto *view = a->m_webContents->GetRenderWidgetHostView();
                if (!view) {
                    a->finishScreenshotRequest(id, QByteArray());
                    return;
                }

                gfx::Size size(sz.width(), sz.height());
                view->CopyFromSurface(
                    gfx::Rect(), size,
                    base::BindOnce([](QWeakPointer<WebContentsAdapter> w2, quint64 id2,
                                      const SkBitmap &bitmap) {
                        auto a2 = w2.toStrongRef();
                        if (!a2)
                            return;
                        QByteArray result;
                        ...
                        a2->finishScreenshotRequest(id2, std::move(result));
                    }, w, id));
            });
        }, weak, reqId, outputSize));

    // 5. Safety timeout (15s)
    QTimer::singleShot(15000, [weak, reqId]() {
        auto a = weak.toStrongRef();
        if (!a)
            return;
        if (a->m_screenshotCallbacks.find(reqId) != a->m_screenshotCallbacks.end())
            a->finishScreenshotRequest(reqId, QByteArray());
    });
}

void WebContentsAdapter::finishScreenshotRequest(quint64 reqId, QByteArray result)
{
    auto it = m_screenshotCallbacks.find(reqId);
    if (it == m_screenshotCallbacks.end())
        return;

    // Clear viewport override → renderer goes back to widget size
    auto *view = m_webContents->GetRenderWidgetHostView();
    if (view) {
        auto *rwhvqt = static_cast<RenderWidgetHostViewQt *>(view);
        rwhvqt->m_captureSizeOverride = gfx::Rect();
        rwhvqt->synchronizeVisualProperties(std::nullopt);
    }

    auto cb = std::move(it->second.callback);
    m_screenshotCallbacks.erase(it);
    // ScopedClosureRunner destructs → DecrementCapturerCount
    cb(std::move(result));
}
