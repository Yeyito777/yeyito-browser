...
bool RenderWidgetHostViewQtDelegateItem::event(QEvent *event)
{
    if (event->type() == QEvent::ShortcutOverride) {
        if (RenderWidgetHostViewQtDelegateClient::s_suppressFocusCount > 0)
            return false;
...
// When s_suppressFocusCount > 0 (screenshot capture in progress), all focus
// and keyboard events are suppressed at this layer.  This prevents transient
// tab switches (needed for rendering background tabs) from disrupting the
// active tab's renderer focus/IME state or delivering keystrokes to the
// wrong renderer.
